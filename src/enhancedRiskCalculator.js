...
class EnhancedRiskCalculator {
  constructor(config = {}) {
    // Initialize the custom factor system with profile-driven multipliers
    this.factorSystem = new CustomEnvironmentalFactorSystem({
      contextMultipliers: config.contextMultipliers || null
    });
...
calculateNormalizedFileScore(severityDistribution) {
  ...
  calculateFileRisk(vulnerabilities, context = {}) {
    ...
    // IMPORTANT: We use our own normalized score, not factorCalculation.finalScore
        ...
        // Step 5: Apply multipliers to RAW score FIRST, then cap at 100
    // This prevents saturation when normalized=100 for single findings
    const rawWithContext = scoreData.raw * (factorCalculation.totalMultiplier || 1);
    const finalScore = Math.min(100, rawWithContext);
