//BACKUP// contextualFilter.js - Contextual False Positive Filtering Module
    ...
    if (this.config.filterInjectionWithoutInput) {
      ...
      if (injectionCheck.shouldFilter) {
        return {
          ...
          reason: injectionCheck.reason || 'injection without user input',
          confidence: injectionCheck.confidence || 0.75,
 ...
 */async checkInjectionContext(finding, projectPath, contextInference) {
  ...
  try {
    ...
    if (!hasUserInput) {
      ...
      signals.details.push('No user input detected in code path');
    }
    ...
    // Signal 3a: Internal/utility code
    if (filepath.includes('util') || 
      ...
      signals.details.push('Located in utility/helper code');
    }
    
    // Signal 3b: Test/example code (but NOT OWASP Benchmark)
    ...
    // Signal 3c: Configuration/setup code
    if (filepath.includes('config') || 
        filepath.includes('setup') ||
        filepath.includes('install')) {
      contextSignalDetected = true;
      signals.details.push('Located in configuration code');
    }
    ...
    // ========================================
    // DECISION LOGIC: Require 2+ independent categories
    // ========================================
    const activeCategoryCount = signals.categories.length;
    
    if (activeCategoryCount >= 2) {
      // HIGH CONFIDENCE: 2+ independent signals agree → FILTER
      return {
        shouldFilter: true,
        action: 'FILTER',
        reason: 'multiple-independent-signals',
        categories: signals.categories,
        details: signals.details.join('; '),
        confidence: 0.85,
        message: `Injection finding with ${activeCategoryCount} independent safe signals: ${signals.categories.join(', ')}`
      };
      
    } else if (activeCategoryCount === 1) {
      // MODERATE CONFIDENCE: 1 signal only → DOWNGRADE
      return {
        shouldFilter: false,
        action: 'DOWNGRADE',
        reason: 'single-safe-signal',
        categories: signals.categories,
        details: signals.details.join('; '),
        confidence: 0.6,
        severityAdjustment: -1,
        priorityAdjustment: 'P2',
        message: `Injection finding with 1 safe signal: ${signals.categories[0]}`
      };
      
    } else {
      // NO EVIDENCE: Keep as-is
      return {
        shouldFilter: false,
        action: 'KEEP',
        categories: [],
        details: 'No safe context signals detected'
      };
    }
...
async detectUserInput(finding, projectPath) {
  try {
    ...
    // Read actual file content instead of relying on finding.code
    const fs = require('fs').promises;
    const fullPath = path.isAbsolute(finding.file)  // ✅ FIXED
      ? finding.file 
    ...
    try {
      ...
      // If file read fails, fall back to filepath-only analysis
    ...
    const httpPatterns = [
      ...
      'getparameter', 'httpservletrequest', 'servletrequest',
      '@requestparam', '@pathvariable', '@requestbody'
    ];
    
    // Input patterns
    const inputPatterns = [
      'form', 'input', 'stdin', 'scanner', 'bufferedreader',
      'readline', 'readfile', 'process.argv'
    ];
    
    // Filepath patterns
    const filepathPatterns = [
      'controller', 'handler', 'endpoint', 'api', 'route', 'servlet'
    ];
  ...
  try {
   ...
   const fullPath = path.isAbsolute(finding.file) 
    ...
    // Pattern 1: Explicitly internal/private
    const internalPatterns = [
      'internal', 'private', 'local', 'admin',
      'backend', 'dao', 'repository', 'service'
    ];
    
    if (internalPatterns.some(p => filepath.includes(p))) {
      return false; // Not internet-facing
    }
    
    // Pattern 2: Public/API endpoints
    const publicPatterns = [
      'public', 'api', 'rest', 'controller', 'endpoint'
    ];
    
    if (publicPatterns.some(p => filepath.includes(p))) {
      return true; // Internet-facing
    }
    ...
    return true; // Conservative: assume internet-facing on error
  }
}
  /**
